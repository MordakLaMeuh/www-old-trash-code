    ...
    function stepByStep(comment,metaPicture)
    {
        ...
        function galleryComplete()
        {
            ...
            if (level == 0)
            {
                if (renewHome > 1)
                {
                    for (rewriting = 0; rewriting <= directoryNumber; rewriting++)
                    {
                        imagesDOM[rewriting] = document.getElementById(directoryName[rewriting].name);
                        imagesDOM[rewriting].innerHTML = stream[rewriting];
                        console.log("rewriting part:"+rewriting+":"+directoryName[rewriting].name);
                    }
                    renewHome = 1;
                }
                else
                {
                    imagesDOM[directoryNumber].innerHTML = stream[directoryNumber];
                    console.log("activation:"+directoryNumber+":"+directoryName[directoryNumber].name);
                }
            }
            directoryName[directoryNumber].state = true;
            directoryNumber++;
            if (!allLoaded && level == 0)    imagesDOM[directoryNumber] = document.getElementById(directoryName[directoryNumber].name);
        }
