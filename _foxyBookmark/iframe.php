...
var iFrame = (function(){						
    ...
    function ObjiFrame(src,x,y,width,height) {					// CONSTRUCTEUR DE L'OBJET.
      document.body.innerHTML+="<iframe src='"+src+"'></iframe>";		// Déclaration d'un iFrame dans le DOM					
      ...
      var identifiantInDOM = classFrames.length-1; 				// identifiant unique de l'iFrame concerné.
            
      this.setPositionOfFrame = function(x,y,width,height) 		{	// Méthode publique de positionnement de l'iFrame;
	classFrames[identifiantInDOM].style.width  = width   +"px";
	classFrames[identifiantInDOM].style.height = height  +"px";
	classFrames[identifiantInDOM].style.marginLeft  = x  +"px";
	classFrames[identifiantInDOM].style.marginTop   = y  +"px";	}  
      
      this.setSrc = function(src) { classFrames[identifiantInDOM ].src=src; }    // Méthode publique du changement de la page affichée dans l'iFrame.
      
      this.returnID = function()  { return identifiantInDOM +"current src:"+classFrames[identifiantInDOM ].src; } 	// Méthode publique...
	...
	document.body.removeChild(classFrames[identifiantInDOM]);		// step1: Effacement de l'iFrame à l'écran.			
    ...
    return ObjiFrame;								// retourne le constructeur lors de la création d'un nouvel objet
})();
...
//titi.setSrc('http://216.58.208.195');
titi.setSrc('http://duckduckgo.com');
toto21.delete();
tata.delete();
var popol=new iFrame('https://www.facebook.com/',100,100,500,400);
toto21=new iFrame('http://www.zebulon.fr/outils/connaitre-mon-ip/',50,50,500,350);
popol.delete();
